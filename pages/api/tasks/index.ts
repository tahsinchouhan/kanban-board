...
const createTask = async (
  ...
  if (taskData.name.length < 1 || taskData.name.length > 120) {
    return res
      .status(400)
      .json({ error: "Task name must be between 1 and 120 characters" });
  }
  ...
  for (const subtask of taskData.subtasks ?? []) {
    if (
      ...
      (subtask.completed && typeof subtask.completed !== "boolean")
    ) {
  ...
  const nextPosition = existingColumnTasks.length
    ? existingColumnTasks[0].position + 1
    : 0;
