...
const TaskForm: FC<{
  ...
  const initialColumn = props.columns?.find(
    (column) => column.uuid === props.taskData?.column_uuid
  )?.name;
  ...
  const descriptionInput = useInput<string>({
    initialValue: props.taskData?.description,
  });
  ...
  const columnDropdown = useInput<string>({
    initialValue: initialColumn ?? (dropdownOptions && dropdownOptions[0]),
  });
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (newColumnsValue)
      subtasksInput.customValueChangeHandler(newColumnsValue);
    if (formIsValid) {
      const formData = {
        name: nameInput.value,
        description: descriptionInput.value,
        subtasks: subtasksInput.value?.map((item) => {
          const subtask = props.taskData?.subtasks?.find(
            (subtask) => subtask.uuid === item.id
          );
          if (subtask) return { uuid: subtask.uuid, name: item.value };
          return { name: item.value };
        }),
        column_uuid: props.columns?.find(
          (item) => item.name === columnDropdown.value
        )?.uuid,
      };
